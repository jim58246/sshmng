...
- **CLI 重构为子命令模式**：`sshmng mcp` / `sshmng install` / `sshmng doctor` / `sshmng help`，无参数弹交互式菜单
...
直接弹菜单，**不做 TTY 自动检测、不退回 MCP 模式**。工具未广泛发布，干净 break。

若非 TTY（如 Agent 误用 `sshmng` 无 args 拉起），报错退出：
```
sshmng requires a TTY for interactive mode.
Use 'sshmng mcp' for MCP server mode (this is what Agent configs should use).
```

### 菜单

```
sshmng — SSH session manager

What do you want to do?
  1. Install    — First-time setup (create config, inject into Agents)
  2. Doctor     — Verify setup
  3. MCP server — Start stdio MCP server (advanced)
  4. Help       — Show full help
Choice [1-4]: 
```
...
旧 Agent 配置（如用户开发期间使用的）写的是 `"command": "sshmng"` 无 args。新结构下 `sshmng` 无 args 弹菜单，旧配置失效。
...
- **菜单字符**：用 ASCII（`[ok]` / `->` / `*`）不用 Unicode `✓→•`——Windows Terminal 支持，但旧 cmd.exe 不支持，统一 ASCII 避免乱码
- **TTY 检测**：`golang.org/x/term.IsTerminal`（跨平台，已支持 Windows）
...
- `golang.org/x/term`（TTY 检测，可能已是间接依赖；wizard 移除后不再需要密码隐藏输入）
...
- **TTY 缺失**（`sshmng` 无 args 但非 TTY）：报错 "requires a TTY for interactive mode"，exit 2
