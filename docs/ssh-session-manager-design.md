...
LoginAction 本质是 send+expect 行为组合：在 SSH auth 完成、shell 就绪后，于 PTY 上发送命令、等待并判断结果。
...
- 入口：`Jumphost.Entry`（或 `SSHServer.Entry`）字段指向首个 Action 名
- 终止：当某 `Expect.Next == "success"` 时，登录成功
- `"success"` 是保留字符串，不能作为 LoginFlow 的 key

**Send / Expects 语义：**

- `Send` 可空：空时跳过发送，直接进入 expect 阶段。典型场景是 Entry Action——SSH auth 完成、shell 就绪后远端先输出一段文字（MOTD / PS1 / 堡垒机菜单），我们等它输出完再行动
...
- **Jumphost** — SSH 跳板，含 LoginFlow 决定交互流程（空 = 透明转发等价于 ssh -J；非空 = 交互式堡垒机菜单）
- **SSHServer** — 目标机，引用 Jumphost 和 Proxy，本身也可有 LoginFlow（目标机认证完成后的交互，如 su / 角色选择 / PAM）

**两种形态的认证链路：**

- **Pattern A（`Jumphost.LoginFlow` 空，透明转发，等价 ssh -J）**：客户端 SSH auth 到 jumphost（`Jumphost.Auth`）→ 客户端经 jumphost 的 direct-tcpip 通道 SSH auth 到 target（`SSHServer.Auth`）→ target shell 就绪 → `SSHServer.LoginFlow`（如有）
- **Pattern B（`Jumphost.LoginFlow` 非空，交互式堡垒机菜单）**：客户端 SSH auth 到 jumphost（`Jumphost.Auth`）→ jumphost shell 就绪 → `Jumphost.LoginFlow` 走菜单 → 菜单完成后 **jumphost 内部连接到 target**，客户端 PTY 直接看到 target shell（客户端不再做 SSH auth 到 target，也不走隧道）→ `SSHServer.LoginFlow`（如有）
- **Pattern B 下 `SSHServer.Auth` 不使用**；菜单若要求 target 凭据，硬编码在 `Jumphost.LoginFlow.Send` 中（v1 接受此限制）
...
- 堡垒机场景本就是 PTY，统一后逻辑一致——堡垒机只是 PTY 的一个子类型（LoginFlow 非空）
- exec 模式下 cwd/env 不保留，对运维连续多命令场景不自然

**Jumphost 两种形态靠 LoginFlow 区分：**

- `LoginFlow` 空 = 无菜单交互，等价于透明转发（ssh -J 语义）
- `LoginFlow` 非空 = 交互式堡垒机，按决策树走完菜单才到目标机
...
type LoginAction struct {
    Name      string
    Send      string             // 支持变量替换 ${FieldName}、转义 \n \r \t
...
type Jumphost struct {
    ...
    User             string
    Auth             SSHAuth
    LoginFlow        map[string]LoginAction  // 空 = 透明转发；非空 = 交互式堡垒机菜单
    Entry            string                // LoginFlow 的起始 Action 名
...
type SSHServer struct {
    ...
    User             string
    Auth             SSHAuth
    LoginFlow        map[string]LoginAction  // 可空，目标机认证完成后的交互流程（如 su/角色选择/PAM）
    Entry            string
    MaxSteps         int
    GlobalTimeoutMs  int
...
- 变量替换：`${FieldName}` 引用 SSHServer 或 Jumphost 的字段（如 `${Password}` `${User}` `${Addr}`）
- 自定义变量不支持（避免演变成脚本语言）
- `LoginAction.Send` 为空时跳过发送，仅 expect（典型场景：Entry Action 等远端 MOTD / 菜单输出）
...
- `Jumphost.Auth` 必填；`SSHServer.Auth` 在 Pattern A（`Via` 为空或 `Via.LoginFlow` 为空）下必填，Pattern B（`Via.LoginFlow` 非空）下可空且被忽略
...
Trace 存原始信息用于诊断：`Output` 是未清洗的 PTY 原始流（含 sentinel、PS1 残留、ANSI 转义），与 `run_in_session` 返回的 `output`（已清洗）区分。`Inputs` 含 send_input 发送的全部文本（包括密码等敏感数据）。
...
7. 统一 PTY 模式 —— 所有连接（含直连）走 PTY 维持 cwd/env，支持登录后交互（su/角色切换）；堡垒机是 PTY 模式的子类型（LoginFlow 非空），不单独建模
8. Jumphost 两种形态靠 LoginFlow 区分 —— 空 LoginFlow = 透明转发（ssh -J 语义），非空 = 交互式堡垒机菜单
9. Host key 验证用 TOFU —— 首次连接记录 key，后续验证；key 变更报错且 `update_*` 不能重置（安全决策需人工确认），见 3.5
10. 终端规范化分两层 —— PTY 级（TERM/宽度/LANG，连接时）与 shell 级（NO_COLOR/PS1/PROMPT_COMMAND/stty/history，LoginFlow 完成后），见 3.7
...
- Trace 的 `Output` 含命令原始输出，`Inputs` 含 `send_input` 发送的文本（可能含密码）
- Trace 仅存内存，`close_session` 后保留 10 分钟自动清理，不落盘
- 同进程内 `get_trace` 需要 sid，跨进程不可见
- 若不可接受密码进 trace，避免用 `send_input` 传密码——改用 NOPASSWD sudo / key 认证
...
`run_in_session` 对所有会话（含直连）走 PTY 以维持 cwd/env，因此终端规范化统一生效。规范化分两层，生效时机不同：

**PTY 级规范化（SSH 连接时，PTY 分配阶段）：**

- `TERM=dumb`：禁用颜色码和光标控制序列
- 终端宽度 120，高度 100
- `LANG=C.UTF-8`（通过 SSH 环境变量请求，或 RC 注入时 export；C.UTF-8 在精简系统/容器也普遍可用，避免 en_US.UTF-8 缺失时的 locale 警告）

这些设置在 LoginFlow 开始前就生效，堡垒机菜单阶段也受益（减少 ANSI 噪声干扰 pattern 匹配）。

**Shell 级规范化（LoginFlow 完成后，拿到 shell 时注入 RC）：**
...
这些设置需要 shell 进程才能执行，LoginFlow 阶段（还在堡垒机菜单）无法注入。

**注入流程时序：**

1. SSH 连接建立，PTY 分配，PTY 级规范化生效
2. 走 LoginFlow（如有），靠用户 pattern 解析菜单
3. LoginFlow 完成，shell 就绪
4. 发送 shell 探测命令（见"Shell 类型探测与降级"），确定 shell 类型
5. 根据 shell 类型注入对应 RC（bash / zsh / 降级路径）
6. 等待首个 PS1 sentinel 出现，确认注入成功
7. session 转入 `idle`，可 `run_in_session`
...
LoginFlow 完成后、RC 注入前，shell 还没被我们控制，不能用 PS1 sentinel 判断命令结束。探测命令自带结束标记：
