...
// DetectShellReady 判断 PTY 流末尾是否出现本 sid 的初始 PS1 sentinel，标志 shell 就绪。
// 新 sentinel 格式（bash/zsh）：`_<rc>__<sid>_<token?>__]# `，token 可空（injectRC 初始 PS1）。
// dash/ash 仍用 `__P_<sid>__> `（无 $(echo _$?) 扩展）—— 由调用方区分 shell 类型。
// 本函数只处理 bash/zsh sentinel；dash/ash 的就绪检测在 injectRC 中用字面量匹配。
// 宽松匹配 token（\w* 允许空或任意 word 字符），用 sid 限制匹配，避免其他 session 误命中。
func DetectShellReady(stream string, sid string) bool {
	re := regexp.MustCompile(`_-?\d+__` + regexp.QuoteMeta(sid) + `_\w*__]#\s*$`)
	return re.MatchString(stream)
}

// ExtractExitCode 从 PTY 流中提取本 sid + 本 token 的 exit code。
// 新 sentinel 格式：`_<rc>__<sid>_<token>__]# `，rc 是 exit code（capture group 1）。
...
func ExtractExitCode(stream string, sid string, token string) (int, bool) {
	re := regexp.MustCompile(`_(-?\d+)__` + regexp.QuoteMeta(sid) + `_` + regexp.QuoteMeta(token) + `__]# `)
